...
with open("skills/ark/s_items.json", "r") as file:
    ...
    s_item_lookup = list(
        enumerate(map(lambda elem: elem["displayName"].lower(), s_items))
    )
...
enabled_channels = [1186805792729485362, 1182692454223315058, 1447978372222161099]


async def try_execute(message):
    # Return if channel is disabled or `ark` is not found in message content
    if (
        message.channel.id not in enabled_channels
        or ark_token not in message.clean_content.lower()
    ):
        return
...
async def send_command_format_reminder(message):
    await message.channel.send(
        "\nThe three numbers represent quantity, quality and blueprint spawning (boolean: 0, 1) in that order."
    )
...
async def lookup_item_command(message, lookup, item_array, search_token):
    ...
    matched_item_indices = list(map(lambda elem: elem[0], matched_item_enumerables))
    matched_items = [item_array[i] for i in matched_item_indices]
...
async def send_instructions(message):
    await message.channel.send(
        f"""Hmm I didn't understand that, available commands with the tagline 'ark' include
```{ark_token} {item_token} {{search term}}
{ark_token} {s_plus_token} {{search term}}
{ark_token} {boss_token}
{ark_token} {password_token}```"""
    )
